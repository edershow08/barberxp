-- BarberXP v68 — data real das ocorrências, editável somente pelo dono

drop function if exists public.apply_team_incident(uuid,text,integer,integer,text);
...
begin
  ...
  if p_occurred_at is not null then
    if v_actor.role <> 'leader' then
      raise exception 'Somente o dono pode alterar a data da ocorrência';
    end if;
    if p_occurred_at > now() then
      raise exception 'A data da ocorrência não pode estar no futuro';
    end if;
    v_created_at := p_occurred_at;
  end if;
...
revoke all on function public.apply_team_incident(uuid,text,integer,integer,text,timestamptz) from public;
grant execute on function public.apply_team_incident(uuid,text,integer,integer,text,timestamptz) to authenticated;
